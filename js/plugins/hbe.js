...
// so adding an empty div and check it when decrption.
    ...
    const decryptButton = document.getElementById('hbe-decrypt-btn');
    const feedbackLabel = document.getElementById('hbe-feedback');
    ...
    const ivSalt = hexToArray(dataElement.dataset['ivsalt']);
    ...
    async function verifyContent(key, content) {
        ...
        if (!result) {
            feedbackLabel.innerText = wrongHashMessage;
    ...
    async function decrypt(decryptKey, iv, hmacKey) {
        ...
        const result = await cryptoObj.subtle.decrypt({
            ...
            hideButton.innerHTML = '<span class="hbe-btn-icon"><i class="fa-solid fa-lock"></i></span>加密文章';
            hideButton.type = 'button';
            hideButton.classList.add("hbe-button");
            hideButton.classList.add("hbe-encrypt-button");
            ...
            feedbackLabel.innerText = wrongPassMessage;
            console.log(e);
            return false;
        });
